...
export default function App() {
    ...
    useScrollRefToId([aboutRef, experienceRef, projectsRef], parentRef, 28 * 4)
    ...
    return (
        ...
        >
            <div
                ...
                <div
                    className="fixed inset-0 pointer-events-none lg:absolute"
                    style={{background: `radial-gradient(600px at ${x}px ${y}px, rgba(101, 163, 13, 0.25), transparent)`}}
                />
                {useMemo(() => (
                        <>
                            <Header/>
                            <div className="flex flex-col col-start-2 gap-y-28 pb-28">
                                ...
                                <Card.CardGroup id="expirience" ref={experienceRef}>
...
function MobileOnlyHeader({children}: { children: ReactNode }) {
    return <h4 className="inline-block lg:hidden text-lg font-semibold mb-7">{children}</h4>
}
